    ...
    if task == 'rgb':
        if args.use_opencv:
            ...
            if args.new_short == 0:
                ...
                # cmd = osp.join(
                #    f"ffmpeg '{full_path}' -r 30 '{out_full_path}/{vid_path[:-4]}/{vid_path[:-4]}-%09d.jpg'")
                call(["ffmpeg", "-i", full_path, "-r", "30", out_full_path+'/'+vid_path[:-4]+'/'+vid_path[:-4]+'-%09d.jpg'])
            else:
                # cmd = osp.join(
                #     f"denseflow '{full_path}' -b=20 -s=0 -o='{out_full_path}'"
                #     f' -ns={args.new_short} -v')
                cmd = osp.join(
                    f"ffmpeg '{full_path}' -r 30 '{out_full_path}/{vid_path[:-4]}")
...
if __name__ == '__main__':
    ...
    if args.level == 2:
        ...
        for classname in classes:
            new_dir = osp.join(args.out_dir, classname[:-4])
