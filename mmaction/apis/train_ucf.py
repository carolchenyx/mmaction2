...
from ..core import (DistEvalHook, EvalHook, OmniSourceDistSamplerSeedHook,
                    OmniSourceRunner, videoEpochBasedRunner)
...
def init_random_seed_ucf(seed=None, device=default_device, distributed=True):
...
def train_model_ucf(model,
    ...
    if cfg.training_method == 'OmniSourceRunner':
        Runner = OmniSourceRunner
    elif cfg.training_method == 'epoch_based':
        Runner = EpochBasedRunner
    else:
        Runner = videoEpochBasedRunner
    runner = Runner(
        model,
        optimizer=optimizer,
